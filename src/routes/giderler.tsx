...
import { TrendingDown, Wallet, Landmark } from "lucide-react";
...
function Page() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Giderler" subtitle="Tüm banka ve kasa giderleri tek tabloda." />

      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Toplam Gider" value={fmtTL(totalBank + totalCash)} icon={TrendingDown} tone="destructive" />
        <StatCard label="Banka Çıkışları" value={fmtTL(totalBank)} icon={Landmark} tone="primary" />
        <StatCard label="Kasa Çıkışları" value={fmtTL(totalCash)} icon={Wallet} tone="info" />
      </div>

      {all.length === 0 ? (
        <EmptyState title="Henüz gider yok" desc="Banka veya Kasa hareketleri girildiğinde otomatik listelenir." />
      ) : (
