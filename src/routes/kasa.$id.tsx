...
function Page() {
  ...
  const openNew = (type: "in" | "out") => {
    setEditing(null);
    setForm({ date: new Date().toISOString().slice(0, 10), description: "", category: "", type, amount: 0 });
    setOpen(true);
  };
  ...
  const save = () => {
    ...
    const signed = form.type === "in" ? Math.abs(form.amount) : -Math.abs(form.amount);
    if (editing) {
      ...
      toast.success(form.type === "in" ? "Giriş kaydedildi" : "Çıkış kaydedildi");
    }
    setOpen(false);
    void flushSync();
  };

  const totalIn = sorted.filter((t) => t.amount >= 0).reduce((a, t) => a + t.amount, 0);
  const totalOut = sorted.filter((t) => t.amount < 0).reduce((a, t) => a + Math.abs(t.amount), 0);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => openNew("in")}>
              <Plus className="mr-1 h-4 w-4" /> Giriş Ekle
            </Button>
            <Button size="sm" variant="destructive" onClick={() => openNew("out")}>
              <Minus className="mr-1 h-4 w-4" /> Çıkış Ekle
            </Button>
          </>
        }
      />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card className="glass"><CardContent className="p-5">
          ...
          <div className="text-2xl font-bold text-success">{fmt(totalIn, cash.currency)}</div>
        </CardContent></Card>
        <Card className="glass"><CardContent className="p-5">
          <div className="text-xs text-muted-foreground">Toplam Çıkış</div>
          <div className="text-2xl font-bold text-destructive">{fmt(totalOut, cash.currency)}</div>
        </CardContent></Card>
      </div>
      ...
      <Card className="glass"><CardContent className="p-4">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {sorted.length === 0 && (
                <TableRow><TableCell colSpan={7} className="py-10 text-center text-sm text-muted-foreground">Henüz hareket yok</TableCell></TableRow>
              )}
              {sorted.map((t) => {
                ...
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    <span className={`rounded-md px-2 py-0.5 text-xs font-semibold ${isIn ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
                      {isIn ? "Giriş" : "Çıkış"}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="text-right font-semibold text-success">
                    {isIn ? `+${fmt(t.amount, cash.currency)}` : "—"}
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => { removeCashTx(t.id); toast.success("Silindi"); void flushSync(); }}>
              ...
              <div>
                ...
                <Select value={form.category || "__none"} onValueChange={(v) => setForm({ ...form, category: v === "__none" ? "" : v })}>
                  ...
                  <SelectContent>
                    ...
                    {["Yemek", "Yakıt", "Kargo", "Market", "Personel", "Ofis", "Reklam", "Diğer"].map((c) => (
